...
import { getAiFrequencyUsingGet } from '@/services/FrankBI/aiFrequencyController';
import { addOrderUsingPost } from '@/services/FrankBI/aiFrequencyOrderController';
import {
  getLoginUserUsingGet,
  getUserVoByIdUsingGet,
  updateByProfileUserUsingPost,
  updateMyInfoUsingPost
} from '@/services/FrankBI/userController';
...
const UserInfo: React.FC = () => {
  ...
  const handleOk = async () => {
    ...
    if (res.data) {
      message.success('生成订单成功，请在个人订单中查看订单信息');
    } else {
  ...
  return (
    <>
      <Divider style={{ fontWeight: 'bold' }}>用户头像</Divider>
      ...
      <Divider style={{ fontWeight: 'bold' }}>用户信息</Divider>
      <Descriptions
        bordered
        column={1}
        size={'default'}
        contentStyle={{ fontWeight: 'bold', maxWidth: '200px' }}
      >
        ...
        <Descriptions.Item style={{ textAlign: 'center' }} label="用户账号：">
          {myUser.userAccount}
        </Descriptions.Item>
        ...
        <Descriptions.Item style={{ textAlign: 'center', color: '#52c41a' }} label="我的状态：">
          {/*@ts-ignore*/}
      ...
      <ModalForm<API.UserUpdateMyRequest>
        ...
        trigger={
          <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
            <Button type="primary" size="large" block style={{ margin: '50px', width: '250px' }}>
              修改个人信息
            </Button>
          </div>
        }
